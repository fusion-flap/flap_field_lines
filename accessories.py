...
def pixel_2_array(xp, yp, x, y, dx=1, dy=1):
    xp = xp - x[0]
    yp = y[-1] - yp
    return np.floor(yp / dy).astype('int'), np.floor(xp / dx).astype('int')
...
def initial_process(shot, time, t0, tend, save_path, f_low=2000, f_high=11000):
    ...
    x, y, _, _ = return_view_xy(data)
    data.data = data.data.swapaxes(0,1)[::-1,:,:]
    data.get_coordinate_object('Image x').start = y[0]
    data.get_coordinate_object('Image y').start = x[0]
